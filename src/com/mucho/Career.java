...
public class Career {
    ...
    public double compareCategory(double[] thisCareerCategory, double[] otherCareerCategory){
        ...
        for(int i = 0; i < shortestCareerLength; i++){
            if ((Double.isNaN(thisCareerCategory[i]) && !(Double.isNaN(otherCareerCategory[i]))) || (!(Double.isNaN(thisCareerCategory[i])) && (Double.isNaN(otherCareerCategory[i])))){
               // if one number is NaN and the other isn't, season similarity is 0
                seasonSimilarity = 0;
               overallSimilarity += seasonSimilarity;
            } else if ((Double.isNaN(thisCareerCategory[i])) && (Double.isNaN(otherCareerCategory[i]))){
                seasonSimilarity = 1;
                overallSimilarity += seasonSimilarity;
            } else {
